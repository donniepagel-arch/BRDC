...
### [2026-06-12] - Checkout Trainer + Cricket Strategy teacher (SHIPPED)
Two practice trainers, both wired into scorer-setup game types ("🎓 Checkout Trainer", "🎓 Cricket Strategy" — solo, bypass the team requirement). sw v87, deployed apex.
**Checkout Trainer** (`public/pages/checkout-trainer-vnext.html`): reuses the EXISTING, verified outshot engine (`public/virtual-darts/js/phase2/outshotTables.js` — loaded as globals; I validated all **162 routes sum correctly + finish on a double**, 0 invalid). Board + two-line aim from the aim game. Drill a number (121/100/81/60/40 chips or any 2–170) or 🎲 Random; the recommended target **glows on the board**; route **re-plans live after every dart** via `getSuggestedShot(remaining, dartsLeft)` (verified: 61→T15/D8, 16→D8, 20→D10, setups for >170, bogey escapes). Per-checkout stats + weak-spot surfacing in localStorage.
**Cricket Strategy** (`public/pages/cricket-strategy-vnext.html`): tappable 7-number board (you/opp marks + scores), live advice + 5 taught lessons (open on 20s → build a lead → stop the bleeding/defense → skip dead numbers → close out to win) + sandbox. **NOTE/FINDING:** the existing `virtual-darts/js/phase2/cricketLogic.js` gives **poor advice** (suggested Bull on an open board, ignored defense, only the endgame call was right) — so I did NOT use it; embedded a **correct** `cricketAdvice(you,opp)` in the page (priority: win-check → defense on opp's scoring number → offense on your closed-they-haven't → develop highest open w/ treble). Verified correct on all 5 lessons. **Backlog: cricketLogic.js is weak — wherever virtual-darts uses its in-game suggestions, they're bad; replace with this advisor.**
NEXT IDEAS: extract the board+scoreDart+sweeper into a shared module (aim-darts + checkout-trainer both inline-copy it now); add a "drill my weak outs" auto-mode; pull real player checkout% from stats to seed the trainer with their actual weak finishes.
Priority: Medium (shipped; cricketLogic replacement + shared-board refactor are the follow-ups)
...
### [2026-06-12] - Glossary page: stats & terms reference (BUILT — verify-passed, ready to deploy)
Built `public/pages/glossary.html` — a BRDC-native page (vNext `vnext-light-page` cream theme + shared `brdc-navigation-init` nav + brand tokens, NOT a standalone /build-app app). 7 cards / 24 terms: **Averages** (3DA, MPR), **Match Structure** (Leg→Set→Match + the `(15)7-2(8)` display format), **X01** (501/301/701, DI, DO, Master Out, Straight, Bust), **Checkouts** (60-99 / 100-139 / 140-160 / 161+ which *requires the bull*, the four 161+ outs, checkout %), **Cricket** (marks, closeouts 5M-9M, bulls), **Standings** (match→set→leg→H2H tiebreak ladder), **Notable Throws** (180/ton/ton-40, 9-mark, cork). Jump-chip TOC at top. Content authored straight from CLAUDE.md RULES 16-20.
Verified on local QA server (port 5612): shared nav renders (Clubhouse tab highlighted), Bebas heading + cream `#fff8ed` bg, 0 console errors, 0 broken jump anchors, 0 missing element IDs, no 375px overflow, screenshot confirmed.
Context: surfaced during vNext testing — the smoke suite tests `/pages/glossary.html` but the page didn't exist → the one smoke "failure." Building it makes that smoke test legitimately pass.
**Remaining:** (1) deploy to apex (PO-gated, RULE 0); (2) the "?" deep-link enhancement — wire a small "?" next to stat labels (3DA/MPR) on dashboard/profile/match-hub that deep-links to the matching glossary anchor (`glossary.html#averages` etc.). Anchors already exist on the page; this is the cross-page wiring follow-up.
Priority: Medium (page done; deploy + "?" wiring remain)

### [2026-06-12] - Dead-button / dead-code sweep across all 32 vNext pages (audit tooling added)
Ran a systematic static sweep for the Play-button bug class (rendered control whose handler is never wired). Added two reusable audit scripts: `scripts/audit-dead-handlers.py` (inline `onclick=` → undefined-function check) and `scripts/audit-uncalled-fns.py` (functions defined but with zero call sites). **Result: the codebase is largely clean.** Shared nav: clean (Play was the only dead control, already fixed). Inline handlers: 128/128 across 32 pages resolve to defined functions (1 false positive: an inline `if`). Uncalled-function scan flagged 11 suspects, of which 9 are false positives (IIFE `initDirectorGate`; cross-module exports like `requireDirectorLogin` used in 5 modules; pure helpers called via `.then`/internally) and **2 are genuine harmless dead code**:
- `home-vnext.js renderRecentCard()` — self-documented no-op ("recentCard no longer on Home; moved to Profile"), guarded by `if(!els.recentCard) return`. Safe to delete.
- `home-vnext.js handleUpNextChallengeAction()` — orphaned leftover from an old design where Home's Up Next card had inline challenge accept/decline buttons; superseded by the badge→Arena flow (real accept/decline lives in `challenge-system.js` `window.brdcChallenge.respondToChallenge`). No UI renders a control that calls it. Safe to delete.
Neither is a user-facing broken button. **No new live dead-buttons found beyond Play.** Cleanup: delete the two dead functions; optionally wire the audit scripts into the smoke/pre-deploy gate so this class of bug can't reappear silently.
Priority: Low (cleanup + optional CI gate; no user impact)

### [2026-06-12] - Clubhouse / messaging: detect dead Firebase session, prompt re-login (don't show stale-cache shell)
`messages-vnext.html` gates the Clubhouse on live `auth.currentUser` after `waitForAuthReady(6500)` → if the session expired but cached UI still renders the signed-in dashboard shell elsewhere, the user lands on "Log in to use the Clubhouse" with no obvious way back in. Add active dead-session detection: when `auth.currentUser` is null but a cached `brdc_session`/local profile exists (the "looks logged in but isn't" state), show an explicit **"Your session expired — tap to sign back in"** re-auth prompt instead of the bare logged-out Clubhouse, and ideally surface it app-wide (not just on this page) so a stale session is caught on any authed surface.
Context: hit repeatedly during 2026-06-12 multi-account testing — Browser 2 showed a cached dashboard but the Clubhouse read `signedIn:false` even after a manual re-login, with no in-app cue that the session was actually dead. **Needs human confirm in the real app** whether this reproduces for normal (non-automation) use, or is an artifact of the Claude-in-Chrome automation context not inheriting the live session — see test note below.
Priority: High (if it reproduces in real use, logged-in users can silently lose Clubhouse + messaging)

### [2026-06-12] - Stand up the Tester lane (parallel multi-account functional testing)
vNext pages load, but full multi-user functional coverage (post in lobby chat from account A → verify it arrives for account B; DM round-trips; presence) is gated on having two genuinely live sessions. Claude-in-Chrome automation tab did not reliably inherit Donnie's live Firebase session (see Clubhouse item above), which blocked the round-trip from this seat. Plan: a Tester lane where a second model/persona authors the flow scripts (Gemini, to preserve Claude usage) and Playwright/headless drives them against a real authed session — refresh the bit-rotted x01/cricket/e2e specs (they target legacy pre-setup-screen pages) and add Clubhouse/messaging multi-account flows. Parallel to the build lane.
Context: "we really should incorporate testers into our pipeline.. parallel would be fantastic" — surfaced 2026-06-12.
Priority: Medium
