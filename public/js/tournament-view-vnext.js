...
function renderHeader() {
    ...
    els.runtime.href = `/pages/tournament-runtime-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}`;
    ...
    if (isMatchmaker) {
        ...
        mingle.href = `/pages/matchmaker-mingle-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}`;
        mingle.textContent = 'Mingle';
        const tv = document.createElement('a');
        tv.className = 'ves-action';
        tv.dataset.matchmakerLink = 'tv';
        tv.href = `/pages/matchmaker-tv-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}`;
...
async function load() {
    if (!tournamentId) throw new Error('No tournament selected — pick one from Events.');
...
load().then(renderAll).catch(error => {
    console[/^No (team|match|tournament) selected/.test(error?.message||'') ? 'info' : 'error']('[tournament-view-vnext]:', error?.message || error);
