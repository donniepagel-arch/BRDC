...
function renderHeader() {
    ...
    els.back.href = `/pages/tournament-view-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}`;
...
function renderVotingPanel() {
    ...
    els.voteOptions.innerHTML = options.map(option => {
...
Promise.all([loadTournament(), loadSession()]).then(() => {
    ...
    console[/^No (team|match|tournament) selected/.test(error?.message||'') ? 'info' : 'error']('[tournament-register-vnext]:', error?.message || error);
...
els.form.addEventListener('submit', async event => {
    ...
    try {
        ...
        els.result.hidden = false;
        els.result.classList.remove('error');
        els.result.innerHTML = `Registration saved. <a href="/pages/tournament-view-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}">Back to tournament</a>`;
