...
function renderMatches() {
    ...
    els.matches.innerHTML = rows.map(match => {
        ...
        return `
            <a class="ltv-match-row ${resultClass}" href="/pages/match-hub-vnext.html?league_id=${leagueId}&match_id=${escapeHtml(match.id)}">
...
function renderPlayers() {
    ...
    els.players.innerHTML = roster.map(player => {
        ...
        return `
            <a class="ltv-player-row" href="/pages/player-profile-vnext.html?player_id=${escapeHtml(player.id)}&league_id=${leagueId}">
...
function renderChat() {
    els.teamChat.href = `/pages/messages-vnext.html?league_id=${encodeURIComponent(leagueId)}&team_id=${encodeURIComponent(teamId)}`;
    els.chat.innerHTML = `
...
async function loadData() {
    if (!teamId) throw new Error('No team selected — open a team from your league standings.');
...
function renderAll() {
    if (els.leagueTeamLink) {
        els.leagueTeamLink.href = `/pages/triples-vnext.html?league_id=${encodeURIComponent(leagueId)}`;
    }
    renderHero();
    renderOverview();
    renderMatches();
...
async function init() {
    ...
    try {
        ...
        console[/^No (team|match|tournament) selected/.test(error?.message||'') ? 'info' : 'error']('[league-team-vnext]:', error?.message || error);
