import { callFunction, auth, waitForAuthReady } from '/js/firebase-config.js';
...
function renderLeagues() {
    ...
    els.leagueList.innerHTML = leagues.length
        ? leagues.map(league => card(league.status || 'League', league.name || league.league_name || 'League', `Director: ${league.director_name || 'Not set'}`, `<a href="/pages/league-director-vnext.html?league_id=${encodeURIComponent(league.id)}">Director board</a><a href="/pages/triples-vnext.html?league_id=${encodeURIComponent(league.id)}">League hub</a>`)).join('')
...
function renderTournaments() {
    ...
    els.tournamentList.innerHTML = tournaments.length
        ? tournaments.map(tournament => card(tournament.status || tournament.registration_status || 'Tournament', tournament.name || tournament.tournament_name || 'Tournament', tournament.format || tournament.game_type || 'Format TBD', `<a href="/pages/tournament-view-vnext.html?tournament_id=${encodeURIComponent(tournament.id)}">Tournament page</a><a href="/pages/tournament-runtime-vnext.html?tournament_id=${encodeURIComponent(tournament.id)}">Runtime</a>`)).join('')
...
async function load() {
    ...
    if (!user) {
        const err = new Error('Admin unavailable');
        err.isAuthGate = true;
        throw err;
    }
...
load().then(renderAll).catch(error => {
    // Quiet for expected non-admin / not-signed-in cases (auth gate or a cloud Unauthorized/permission error):
    // the UI degrades gracefully to "Admin unavailable", so don't log a scary console error for them.
    const quiet = error.isAuthGate || /unauthorized|permission|denied|not.?authorized/i.test(error.message || '');
    if (!quiet) {
