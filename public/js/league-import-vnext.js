...
async function importRecap() {
    ...
    try {
        ...
        window.location.href = `/pages/match-hub-vnext.html?league_id=${encodeURIComponent(leagueId)}&match_id=${encodeURIComponent(matchId)}`;
    } catch (error) {
...
async function init() {
    if (leagueId) {
        els.manageLeagueLink.href = `/pages/triples-vnext.html?league_id=${encodeURIComponent(leagueId)}#manage`;
    }
    resetPreview();
