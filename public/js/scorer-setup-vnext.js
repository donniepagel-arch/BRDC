...
function appendSharedParams(params, homePlayers, awayPlayers) {
    ...
    params.set('return_url', '/pages/scorer-setup-vnext.html');
...
function scorerUrlForGame(game, params) {
    if (game === 'cricket') return `/pages/league-cricket-vnext.html?${params}`;
    return `/pages/x01-scorer-vnext.html?${params}`;
}
...
async function createKnockoutBracket() {
    ...
    try {
        ...
        const result = await callFunction('createKnockout', {
            name: els.knockoutName.value.trim() || 'BRDC Knockout',
            teams,
            format,
        ...
        window.location.href = `/pages/knockout.html?id=${encodeURIComponent(result.knockout_id)}`;
    } catch (error) {
...
function resumeMatch() {
    ...
    window.location.href = `/pages/x01-scorer-vnext.html?${params}`;
}
