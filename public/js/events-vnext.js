...
function hrefFor(event) {
    if (event.type === 'league') return `/pages/match-hub-vnext.html?league_id=${LEAGUE_ID}&match_id=${encodeURIComponent(event.id)}`;
    if (event.type === 'tournament') return `/pages/tournament-view-vnext.html?tournament_id=${encodeURIComponent(event.id)}`;
    return event.href || '/pages/events-vnext.html';
}
...
async function loadEvents() {
    ...
    const tournamentEvents = tournamentsSnap.docs.map(doc => {
        const data = doc.data();
        return {
            id: doc.id,
            type: 'tournament',
