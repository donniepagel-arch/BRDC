...
function setMode(mode) {
    ...
    document.querySelectorAll('[data-tv-mode]').forEach(link => {
        ...
        link.href = `/pages/matchmaker-tv-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}&mode=${encodeURIComponent(link.dataset.tvMode)}`;
    });
...
init().catch(error => {
    console[/^No (team|match|tournament) selected/.test(error?.message||'') ? 'info' : 'error']('[matchmaker-tv-vnext]:', error?.message || error);
