...
const openLeagueLink = document.getElementById('openLeagueLink');
...
const DEFAULT_TIEBREAKER_ORDER = ['match_wins', 'set_wins', 'leg_wins', 'head_to_head', 'point_differential', 'total_points'];

let tiebreakerOrder = [...DEFAULT_TIEBREAKER_ORDER];

function renderTiebreakerList() {
    const container = document.getElementById('tiebreakerList');
    if (!container) return;
    container.innerHTML = '';
    tiebreakerOrder.forEach((key, index) => {
        const row = document.createElement('div');
        row.className = 'cl-tiebreaker-row';
        row.dataset.key = key;
        row.innerHTML = `
            <span class="cl-tb-priority">${index + 1}</span>
            <span class="cl-tb-label">${TIEBREAKER_LABELS[key] || key}</span>
            <button class="cl-tb-btn" type="button" data-tb-up aria-label="Move ${TIEBREAKER_LABELS[key] || key} up"${index === 0 ? ' disabled' : ''}>&#9650;</button>
            <button class="cl-tb-btn" type="button" data-tb-down aria-label="Move ${TIEBREAKER_LABELS[key] || key} down"${index === tiebreakerOrder.length - 1 ? ' disabled' : ''}>&#9660;</button>
        `;
        row.querySelector('[data-tb-up]').addEventListener('click', () => moveTiebreaker(index, -1));
        row.querySelector('[data-tb-down]').addEventListener('click', () => moveTiebreaker(index, 1));
...
function moveTiebreaker(index, direction) {
    ...
    if (next < 0 || next >= tiebreakerOrder.length) return;
    [tiebreakerOrder[index], tiebreakerOrder[next]] = [tiebreakerOrder[next], tiebreakerOrder[index]];
    renderTiebreakerList();
}

function setTiebreakerOrder(order) {
    if (!Array.isArray(order) || !order.length) return;
    const known = new Set(DEFAULT_TIEBREAKER_ORDER);
    const incoming = order.filter(k => known.has(k));
    const missing = DEFAULT_TIEBREAKER_ORDER.filter(k => !incoming.includes(k));
    tiebreakerOrder = [...incoming, ...missing];
    renderTiebreakerList();
}

// ── Blackout date chips UI ───────────────────────────────────────────────────
let blackoutDates = [];

function renderBlackoutChips() {
    const container = document.getElementById('blackoutChips');
    if (!container) return;
    container.innerHTML = '';
    blackoutDates.forEach(date => {
        const chip = document.createElement('span');
        chip.className = 'cl-chip';
        chip.innerHTML = `${escapeHtml(date)}<button class="cl-chip-remove" type="button" aria-label="Remove ${escapeHtml(date)}">&times;</button>`;
        chip.querySelector('.cl-chip-remove').addEventListener('click', () => removeBlackoutDate(date));
        container.appendChild(chip);
    });
}

function addBlackoutDate() {
    const picker = document.getElementById('blackoutDatePicker');
    if (!picker) return;
    ...
    blackoutDates = [...blackoutDates, value].sort();
    picker.value = '';
    renderBlackoutChips();
}
...
function setBlackoutDates(dates) {
    if (Array.isArray(dates)) {
        blackoutDates = dates.filter(Boolean).map(String).sort();
    } else if (typeof dates === 'string' && dates.trim()) {
        blackoutDates = dates.split(',').map(d => d.trim()).filter(Boolean).sort();
    } else {
        blackoutDates = [];
    }
    renderBlackoutChips();
...
function initBlackoutUI() {
    const btn = document.getElementById('blackoutAddBtn');
    const picker = document.getElementById('blackoutDatePicker');
    btn?.addEventListener('click', addBlackoutDate);
    picker?.addEventListener('keydown', e => { if (e.key === 'Enter') { e.preventDefault(); addBlackoutDate(); } });
}
...
function collectLeaguePayload(data) {
    ...
    // Canonical day key: write both match_day (vNext) and league_night (readers)
    const matchDay = data.match_day || 'wednesday';
    return {
        name,
        league_name: name,
        season,
...
function populateForm(league) {
    ...
    setBlackoutDates(league.blackout_dates || []);
    ...
    setField('cork_option', league.cork_option || 'alternate_random_first_last');
    setField('cork_winner_gets', league.cork_winner_gets || 'choose-and-start');
    setTiebreakerOrder(Array.isArray(league.tiebreakers) ? league.tiebreakers : splitCsv(league.tiebreakers || ''));
...
function hasCorkChoiceRound() {
    return roundRows().some(row => {
        const round = row.dataset.round;
        const game = row.querySelector(`[name="round_${round}_game"]`)?.value || '';
        return game === 'corks_choice' || game === 'mixed' || game === 'custom';
    });
}

function updateCorkWinnerGetsVisibility() {
    const wrap = document.getElementById('corkWinnerGetsWrap');
    if (wrap) wrap.hidden = !hasCorkChoiceRound();
}
...
function initRoundBuilder() {
    ...
    roundsEl?.addEventListener('change', event => {
        ...
        renderRoundMixed(row);
        updateCorkWinnerGetsVisibility();
    });
    roundsEl?.addEventListener('click', clickEvent => {
        const row = clickEvent.target.closest?.('.ves-round-row');
        if (!row) return;
        if (clickEvent.target.matches('[data-edit-legs]')) {
            row.dataset.mixedOpen = row.dataset.mixedOpen === 'true' ? 'false' : 'true';
            renderRoundMixed(row);
        }
        if (clickEvent.target.matches('[data-add-leg]')) {
        ...
        if (clickEvent.target.matches('[data-remove-leg]')) {
            const legRow = clickEvent.target.closest('.ves-mixed-leg-row');
            legRow?.remove();
...
function currentLeagueUserOrNull() {
    return auth.currentUser || null;
}

function escapeLeagueLoadText(str) {
    return String(str || '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
}

async function saveLeagueDraft() {
    const user = currentLeagueUserOrNull();
    if (!user) { alert('You must be signed in to save a draft.'); return; }
    const raw = prompt('Draft name (leave blank for auto):', form?.querySelector('[name="name"]')?.value || '') ?? '';
    const draftName = raw.trim() || `Draft – ${new Date().toLocaleString()}`;
    setStatus('Saving draft…');
    try {
        const data = Object.fromEntries(new FormData(form).entries());
        const docRef = doc(collection(db, 'league_drafts'));
        await setDoc(docRef, {
            player_id: user.uid,
            owner_email: user.email || '',
            draft_name: draftName,
            draft_data: collectLeaguePayload(data),
            created_at: serverTimestamp(),
            updated_at: serverTimestamp()
        });
        setStatus(`Draft "${draftName}" saved.`, 'success');
    } catch (err) {
        setStatus(err.message || 'Could not save draft.', 'error');
    }
}

async function saveLeagueTemplate() {
    const user = currentLeagueUserOrNull();
    if (!user) { alert('You must be signed in to save a template.'); return; }
    const raw = prompt('Template name:', form?.querySelector('[name="name"]')?.value || '') ?? '';
    const templateName = raw.trim();
    if (!templateName) { setStatus('Template save cancelled.'); return; }
    setStatus('Saving template…');
    try {
        const data = Object.fromEntries(new FormData(form).entries());
        const docRef = doc(collection(db, 'league_templates'));
        await setDoc(docRef, {
            player_id: user.uid,
            owner_email: user.email || '',
            template_name: templateName,
            template_data: collectLeaguePayload(data),
            public: false,
            shareable_to_owner: true,
            created_at: serverTimestamp()
        });
        setStatus(`Template "${templateName}" saved.`, 'success');
    } catch (err) {
        setStatus(err.message || 'Could not save template.', 'error');
    }
}

let clLoadModal = null;

async function openLeagueLoadModal() {
    const user = currentLeagueUserOrNull();
    ...
    try {
        const draftsSnap = await getDocs(
            query(collection(db, 'league_drafts'), where('player_id', '==', user.uid), orderBy('updated_at', 'desc'))
        );
        draftsSnap.forEach(d => items.push({ id: d.id, kind: 'draft', ...d.data() }));
        const templatesSnap = await getDocs(
            query(collection(db, 'league_templates'), where('player_id', '==', user.uid), orderBy('created_at', 'desc'))
        );
        templatesSnap.forEach(d => items.push({ id: d.id, kind: 'template', ...d.data() }));
        setStatus('');
    } catch (err) {
        setStatus(err.message || 'Could not load saved items.', 'error');
        return;
    }

    if (clLoadModal) clLoadModal.remove();
    clLoadModal = document.createElement('div');
    clLoadModal.className = 'cl-load-modal-backdrop';
    clLoadModal.innerHTML = `
        <div class="cl-load-modal" role="dialog" aria-modal="true" aria-label="Load saved league">
            <div class="cl-load-modal-header">
                <strong>Load saved</strong>
                <button class="cl-load-close" type="button" aria-label="Close">&times;</button>
            </div>
            ${items.length === 0
                ? '<p class="cl-load-empty">No saved drafts or templates found.</p>'
                : `<ul class="cl-load-list">${items.map((item, idx) => `
                    <li class="cl-load-item" data-idx="${idx}">
                        <span class="cl-load-badge ${item.kind}">${item.kind === 'draft' ? 'Draft' : 'Template'}</span>
                        <span class="cl-load-name">${escapeLeagueLoadText(item.draft_name || item.template_name || 'Untitled')}</span>
                        <button class="cl-load-btn" type="button" data-idx="${idx}">Load</button>
                    </li>`).join('')}
                </ul>`
            }
        </div>
    `;
    document.body.appendChild(clLoadModal);
    clLoadModal.querySelector('.cl-load-close').addEventListener('click', () => clLoadModal.remove());
    clLoadModal.addEventListener('click', e => { if (e.target === clLoadModal) clLoadModal.remove(); });
    clLoadModal.querySelectorAll('.cl-load-btn').forEach(btn => {
        btn.addEventListener('click', () => {
            const item = items[parseInt(btn.dataset.idx, 10)];
            if (!item) return;
            populateForm(item.draft_data || item.template_data || {});
            clLoadModal.remove();
            setStatus(`Loaded: ${item.draft_name || item.template_name || 'item'}`, 'success');
        });
    });
}
...
function configureMode() {
    if (isSettingsMode) {
        document.title = 'League Settings - BRDC';
        ...
        cancelLink.href = `/pages/triples-vnext.html?league_id=${encodeURIComponent(existingLeagueId)}#manage`;
        if (openLeagueLink) {
            openLeagueLink.href = `/pages/triples-vnext.html?league_id=${encodeURIComponent(existingLeagueId)}#manage`;
            openLeagueLink.hidden = false;
        }
    } else {
        document.title = 'Create League - BRDC';
    }
}

document.getElementById('cl-save-draft-btn')?.addEventListener('click', saveLeagueDraft);
document.getElementById('cl-save-template-btn')?.addEventListener('click', saveLeagueTemplate);
document.getElementById('cl-load-btn')?.addEventListener('click', openLeagueLoadModal);
...
form?.addEventListener('submit', async submitEvent => {
    ...
    const id = isSettingsMode ? existingLeagueId : slugify(name || `${season}-${format}`);
    ...
    try {
        ...
        const directorEmail = directorSession?.email || currentUser?.email || '';
        const directorPlayerId = directorSession?.id || directorSession?.player_id || currentUser?.uid || null;
        Object.assign(payload, {
            director_name: directorSession?.name || currentUser?.displayName || '',
            director_email: directorEmail,
            director_phone: directorSession?.phone || '',
            director_player_id: directorPlayerId,
            owner_email: directorEmail,
            owner_player_id: directorPlayerId
        });
        const result = await callFunction('saveLeague', { league_id: id, payload });
        if (!result?.success) throw new Error(result?.error || 'Could not save league setup.');
        const savedId = result.league_id || id;
        setStatus(isSettingsMode ? 'League settings saved.' : 'Draft league created. Opening league management.', 'success');
        if (!isSettingsMode) window.location.href = `/pages/triples-vnext.html?league_id=${encodeURIComponent(savedId)}#manage`;
    } catch (error) {
