...
const tournamentId = params.get('tournament_id') || params.get('id') || null;
...
function renderHeader() {
    ...
    els.event.href = `/pages/tournament-view-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}`;
    ...
    els.tv.href = `/pages/matchmaker-tv-vnext.html?tournament_id=${encodeURIComponent(tournamentId)}&mode=mingle-alert`;
}
...
load().then(renderAll).catch(error => {
    console[/^No (team|match|tournament) selected/.test(error?.message||'') ? 'info' : 'error']('[matchmaker-mingle-vnext]:', error?.message || error);
