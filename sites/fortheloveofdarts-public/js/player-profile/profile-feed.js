...
const leaguePlayerStatsCache = new Map();
...
function get3DA(stats) {
    ...
    if (toFiniteNumber(stats.x01_total_darts) > 0 && toFiniteNumber(stats.x01_total_points) != null) {
        return (Number(stats.x01_total_points) / Number(stats.x01_total_darts)) * 3;
    }

    const legacyFields = ['avg_3da', 'x01_3da', 'x01_avg', 'three_dart_avg', 'ppd', 'avg'];
...
function getMPR(stats) {
    ...
    if (toFiniteNumber(stats.cricket_total_rounds) > 0 && toFiniteNumber(stats.cricket_total_marks) != null) {
        return Number(stats.cricket_total_marks) / Number(stats.cricket_total_rounds);
    }
...
function getTeam3DA(roster = []) {
    const values = roster.map(get3DA).filter(v => Number.isFinite(v));
    if (values.length === 0) return null;
    return values.reduce((sum, value) => sum + value, 0) / values.length;
}

function getTeamMPR(roster = []) {
    const values = roster.map(getMPR).filter(v => Number.isFinite(v));
    if (values.length === 0) return null;
    return values.reduce((sum, value) => sum + value, 0) / values.length;
}
...
async function getLeaguePlayerStatsMap(leagueId) {
    if (!leagueId) return new Map();
    if (leaguePlayerStatsCache.has(leagueId)) {
        return leaguePlayerStatsCache.get(leagueId);
    }
    ...
    try {
        ...
        statsSnap.forEach(docSnap => {
            ...
            statsMap.set(docSnap.id, merged);
            ...
            [
                ...
                buildNameVariants(name).forEach(variant => {
                    statsMap.set(variant, merged);
                });
            });
        });
    } catch (error) {
        console.warn('Profile feed stats hydration failed for league', leagueId, error?.message || error);
    }

    leaguePlayerStatsCache.set(leagueId, statsMap);
    return statsMap;
}

function hydrateRosterWithStats(roster = [], statsMap = new Map()) {
    return roster.map(player => {
        if (!player) return player;

        const byId = player.id ? statsMap.get(player.id) : null;
        const byName = buildNameVariants(player.name)
            .map(variant => statsMap.get(variant))
            .find(Boolean);
        ...
        return {
            ...player,
            id: player.id || stats.id || null,
            name: player.name || stats.name || stats.player_name || 'Player',
            level: player.level || stats.level || null,
            x01_three_dart_avg: get3DA(player) ?? get3DA(stats),
            cricket_mpr: getMPR(player) ?? getMPR(stats),
            avg_3da: get3DA(player) ?? get3DA(stats),
            mpr: getMPR(player) ?? getMPR(stats)
        };
    });
}

function buildRosterFromMatch(match = {}, side = 'home') {
    const lineup = Array.isArray(match[`${side}_lineup`]) ? match[`${side}_lineup`] : [];
    const roster = lineup.map(player => ({
    ...
    const fallbackNames = Array.isArray(match[`${side}_players`]) ? match[`${side}_players`] : [];
    return fallbackNames
        .map(player => typeof player === 'string' ? { name: player } : {
            id: player?.player_id || player?.id || null,
            name: player?.name || player?.player_name || '',
            level: player?.level || null,
            x01_three_dart_avg: player?.x01_three_dart_avg ?? player?.avg_3da ?? null,
...
function itemNeedsStatHydration(item) {
    if (!item?.data) return false;
    const data = item.data;

    const homeTeamReady = get3DA(data.home_team_avg) != null || getMPR(data.home_team_avg) != null;
    const awayTeamReady = get3DA(data.away_team_avg) != null || getMPR(data.away_team_avg) != null;
    const homeRosterReady = (data.home_roster || []).some(player => get3DA(player) != null || getMPR(player) != null);
    const awayRosterReady = (data.away_roster || []).some(player => get3DA(player) != null || getMPR(player) != null);

    return !(homeTeamReady && awayTeamReady && homeRosterReady && awayRosterReady);
}

async function hydrateMatchResultItem(item) {
    if (!item?.data || !itemNeedsStatHydration(item)) return item;

    const leagueId = item.league_id || item._leagueId;
    const matchId = item.match_id;
    if (!leagueId || !matchId) return item;

    try {
        const [matchSnap, statsMap] = await Promise.all([
            getDoc(doc(db, 'leagues', leagueId, 'matches', matchId)),
            getLeaguePlayerStatsMap(leagueId)
        ]);

        if (!matchSnap.exists()) return item;

        const match = matchSnap.data() || {};
        const homeRoster = hydrateRosterWithStats(
            (item.data.home_roster && item.data.home_roster.length > 0 ? item.data.home_roster : buildRosterFromMatch(match, 'home')),
            statsMap
        );
        const awayRoster = hydrateRosterWithStats(
            (item.data.away_roster && item.data.away_roster.length > 0 ? item.data.away_roster : buildRosterFromMatch(match, 'away')),
            statsMap
        );
        ...
        return {
            ...
            data: {
                ...
                home_team_avg: {
                    ...(item.data.home_team_avg || {}),
                    avg_3da: get3DA(item.data.home_team_avg) ?? getTeam3DA(homeRoster),
                    mpr: getMPR(item.data.home_team_avg) ?? getTeamMPR(homeRoster)
                },
                away_team_avg: {
                    ...(item.data.away_team_avg || {}),
                    avg_3da: get3DA(item.data.away_team_avg) ?? getTeam3DA(awayRoster),
                    mpr: getMPR(item.data.away_team_avg) ?? getTeamMPR(awayRoster)
                }
            }
        };
    } catch (error) {
        console.warn('Profile feed match hydration failed', leagueId, matchId, error?.message || error);
        return item;
    }
}
...
export async function loadFeedTab(state) {
    ...
    const hydratedFeedItems = await Promise.all(allFeedItems.map(item => hydrateMatchResultItem(item)));

    // Sort by date descending
    hydratedFeedItems.sort((a, b) => {
    ...
    if (hydratedFeedItems.length > 0) {
        sections.push(renderLeagueFeedSection(hydratedFeedItems));
    }
...
function renderMatchResultCard(item, idPrefix = 'pf-feed-') {
    ...
    const sortByLevel = (a, b) => {
        const aOrder = levelOrder[rosterLevel(a)] ?? 9;
        const bOrder = levelOrder[rosterLevel(b)] ?? 9;
        if (aOrder !== bOrder) return aOrder - bOrder;
        return String(a?.name || '').localeCompare(String(b?.name || ''));
    };
    const homeRoster = [...(d.home_roster || [])].sort(sortByLevel);
    const awayRoster = [...(d.away_roster || [])].sort(sortByLevel);
    const homeTeam3DA = get3DA(d.home_team_avg) ?? getTeam3DA(homeRoster);
    const homeTeamMPR = getMPR(d.home_team_avg) ?? getTeamMPR(homeRoster);
    const awayTeam3DA = get3DA(d.away_team_avg) ?? getTeam3DA(awayRoster);
    const awayTeamMPR = getMPR(d.away_team_avg) ?? getTeamMPR(awayRoster);
    ...
    for (let i = 0; i < maxRows; i++) {
        ...
        const hL = hFillIn ? 'F' : (hp ? String(hp.level || '').toUpperCase() : '');
        const aL = aFillIn ? 'F' : (ap ? String(ap.level || '').toUpperCase() : '');
        const hBadge = ['A','B','C','F'].includes(hL) ? `<span class="feed-level-badge level-${hL.toLowerCase()}">${hL}</span> ` : '';
        const aBadge = ['A','B','C','F'].includes(aL) ? `<span class="feed-level-badge level-${aL.toLowerCase()}">${aL}</span> ` : '';
        const h3DA = get3DA(hp);
        const hMPR = getMPR(hp);
        const a3DA = get3DA(ap);
        const aMPR = getMPR(ap);
