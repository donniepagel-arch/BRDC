...
const demoNoWriteFunctions = new Set([
    'addbottotournament',
    'creategroupchatroom',
    'createknockout',
    'createleague',
    ...
    'createpost',
    'createtournament',
    'deleteTournament'.toLowerCase(),
    'endmingleperiod',
    'generateblinddrawbracket',
    'generatebracket',
    ...
    'respondtournamentmatchresult',
    'runCupidShuffle'.toLowerCase(),
    'sendchatmessage',
    'senddirectmessage',
    'senddirectorbroadcast',
    'sendtournamentruntimereminder',
    'setpresenceStatus'.toLowerCase(),
    'settournamentmatchroomlifecycle',
    'settournamentparticipantavailability',
    'settournamentplayercheckin',
    'startconversation',
...
function rookiesDemoSession() {
    if (!window.location.pathname.startsWith('/rookies')) return null;
    try {
...
function demoPlayerFromSession(session = {}) {
    const playerId = session.player_id || session.id || 'demo_brian_beach';
    return {
        id: playerId,
        player_id: playerId,
        name: session.name || 'Brian Beach',
        email: session.email || 'brian@rookies.example',
        phone: session.phone || '555-0100',
        source_type: 'rookies_demo',
        league_id: session.league_id || 'rookies-demo-2026-triples',
        is_admin: true,
...
function demoFunctionResponse(functionName, data = {}) {
    const session = rookiesDemoSession();
    if (!session) return null;

    const name = String(functionName || '').toLowerCase();
    if (name === 'getplayersession') {
        return {
            success: true,
            demo: true,
            player: demoPlayerFromSession(session)
        };
    }

    if (!demoNoWriteFunctions.has(name)) return null;

    return {
        success: true,
        demo: true,
        no_write: true,
        message: 'Saved in the Rookies workspace. No live notifications were sent.',
        tournament_id: data.tournament_id || data.tournamentId || data.id || 'rookies-demo-event',
        league_id: data.league_id || data.leagueId || 'rookies-demo-2026-triples',
        match_id: data.match_id || data.matchId || null,
        created_id: data.tournament_id || data.league_id || data.id || 'rookies-demo-no-write'
    };
}
...
async function callFunction(functionName, data = {}) {
    const demoResponse = demoFunctionResponse(functionName, data);
