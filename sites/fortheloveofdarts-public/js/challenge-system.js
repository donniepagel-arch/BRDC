...
import { db, collection, query, where, onSnapshot, callFunction, auth, waitForAuthReady } from './firebase-config.js';
...
let currentPlayerReadyPromise = null;

async function ensureCurrentPlayer(force = false) {
    if (currentPlayerId && !force) {
        return { id: currentPlayerId, name: currentPlayerName };
    }

    if (currentPlayerReadyPromise && !force) {
        return currentPlayerReadyPromise;
    }

    currentPlayerReadyPromise = (async () => {
        const cachedSession = JSON.parse(localStorage.getItem('brdc_session') || '{}');
        if (cachedSession.player_id) {
            currentPlayerId = cachedSession.player_id;
            currentPlayerName = cachedSession.player?.name || cachedSession.player_name || null;
            return { id: currentPlayerId, name: currentPlayerName };
        }

        const activeUser = auth.currentUser || await waitForAuthReady(5000);
        if (!activeUser) {
        ...
        try {
            ...
            if (result?.success && result.player?.id) {
                currentPlayerId = result.player.id;
                currentPlayerName = result.player.name || null;
            ...
            console.warn('[ChallengeSystem] Could not resolve player session:', error?.message || error);
        }
    ...
    try {
        return await currentPlayerReadyPromise;
    } finally {
        currentPlayerReadyPromise = null;
    }
}
...
export async function openChallengeModal(playerId, playerName) {
    ...
    const viewer = await ensureCurrentPlayer();
    if (!viewer.id) {
        toastWarning('Please log in to send challenges');
        return;
    }

    if (playerId === viewer.id) {
