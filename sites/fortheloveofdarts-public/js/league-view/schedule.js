...
import { state, ensurePlayersLoaded, ensureStatsLoaded, ensureTeamRecordsCalculated, getTeamRoster, getTeamStanding } from '/js/league-view/app.js?v=4';
import { getOrdinal, getTeamName, getPlayerName, getTeamRecord } from '/js/league-view/helpers.js?v=4';
...
function getRosterLevel(player) {
    const raw = String(player?.skill_level || player?.level || player?.preferred_level || '').toUpperCase();
    return ['A', 'B', 'C'].includes(raw) ? raw : (player?.isFillIn || player?.is_sub || player?.is_fill_in ? 'F' : '');
}

function sortRosterByLevel(a, b) {
    const order = { A: 0, B: 1, C: 2, F: 3, '': 9 };
    const aOrder = order[getRosterLevel(a)] ?? 9;
    const bOrder = order[getRosterLevel(b)] ?? 9;
...
export async function loadLeagueMatchCard(idx, match) {
    ...
    try {
        ...
        if (matchStatus === 'completed' || (matchStatus === 'scheduled' && (homeLineup.length > 0 || awayLineup.length > 0))) {
            ...
            homeDisplayRoster.sort(sortRosterByLevel);
            awayDisplayRoster.sort(sortRosterByLevel);
        }

        // Do not try to infer fill-ins from historical game names when lineup arrays are missing.
        // That heuristic can pull players onto the wrong side when old imports contain reversed sides,
        // stale unsaved games, or name collisions. Schedule cards should trust saved lineups or fall
        // back to the actual team rosters, not guess.
        ...
        for (let i = 0; i < maxPlayers; i++) {
            ...
            const homeFillIn = Boolean(homePlayer && (homePlayer.isFillIn || homePlayer.is_fill_in || homePlayer.fill_in || homePlayer.team_id === 'fill_in' || homePlayer.team_name === 'Fill-In'));
            const awayFillIn = Boolean(awayPlayer && (awayPlayer.isFillIn || awayPlayer.is_fill_in || awayPlayer.fill_in || awayPlayer.team_id === 'fill_in' || awayPlayer.team_name === 'Fill-In'));
            ...
            const homeLevel = homeFillIn ? 'F' : (homePlayer?.level || '');
            const awayLevel = awayFillIn ? 'F' : (awayPlayer?.level || '');
