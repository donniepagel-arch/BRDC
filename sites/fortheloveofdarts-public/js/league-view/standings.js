...
import { state, ensurePlayersLoaded, ensureStatsLoaded, ensureTeamRecordsCalculated, getTeamRoster } from '/js/league-view/app.js?v=4';

function getPlayoffCutoff() {
    const leagueName = String(state.leagueData?.league_name || state.leagueData?.name || '').toLowerCase();
    if (state.leagueId === 'aOq4Y0ETxPZ66tM1uUtP' || leagueName.includes('triples league') || leagueName.includes('triples draft')) {
        return 6;
    }

    const playoffFormat = state.leagueData?.playoff_format || '';
    if (playoffFormat === 'top_8_single') return 8;
    if (playoffFormat === 'top_6_single') return 6;
    if (playoffFormat === 'top_4_single' || playoffFormat === 'top_4_double') return 4;
...
export function renderStandingsTable() {
    ...
    const teamsWithStats = state.teams.map(t => {
        const rec = state.standingsData.teamStats[t.id] || { wins: 0, losses: 0, gamesWon: 0, gamesLost: 0, legsWon: 0, legsLost: 0 };
        const players = getTeamPlayers(t);

        // Calculate team averages from player stats
        let totalMPR = 0, totalAvg = 0, mprCount = 0, avgCount = 0;
        players.forEach(p => {
        ...
        const teamMPR = mprCount > 0 ? totalMPR / mprCount : 0;
        const teamAvg = avgCount > 0 ? totalAvg / avgCount : 0;
...
export function sortTeams(teamsArray) {
    ...
    return [...teamsArray].sort((a, b) => {
        ...
        switch (col) {
            case 'rank':
            case 'wins':
                // Canonical Triples standings rule: night wins determine rank;
                // sets/points are the first tiebreaker.
...
export function getTeamPlayers(team) {
    ...
    const sortByLevel = (a, b) => {
        const aOrder = levelOrder[playerLevel(a)] ?? 9;
        const bOrder = levelOrder[playerLevel(b)] ?? 9;
    ...
    if (team.players?.length) {
        return [...team.players].sort(sortByLevel).map(p => ({
            id: p.id,
            name: p.name || 'Unknown',
            level: playerLevel(p),
            isCaptain: team.captain_id === p.id
...
export function renderByTeamView(container, sortedTeams) {
    ...
    const playoffCutoff = getPlayoffCutoff();
    ...
    const headerHtml = `
        <div class="standings-controls">
            ...
            ${playoffCutoff > 0 ? `<div class="standings-playoff-note">Top ${playoffCutoff} teams highlighted for playoffs</div>` : ''}
        </div>
    `;
...
export function renderTeamRow(team, idx = 0) {
    ...
    const playoffCutoff = getPlayoffCutoff();
    const isPlayoffTeam = playoffCutoff > 0 && team.rank <= playoffCutoff;
    const rowClass = `${idx % 2 === 0 ? 'row-even' : 'row-odd'}${isPlayoffTeam ? ' playoff-team' : ''}`;
        ...
        <tr class="standings-team-row ${isExpanded ? 'expanded' : ''} ${rowClass}" data-team-id="${team.id}" onclick="toggleTeamExpand('${team.id}', event)">
            ...
            <td>
                <div class="team-name-cell">
                    ...
                    ${isPlayoffTeam ? '<span class="playoff-badge">PLAYOFF</span>' : ''}
                </div>
            </td>
    ...
    const playerRowsHtml = `
        ...
        const playerMatchWins = stats.matches_won ?? wins;
        const playerMatchLosses = stats.matches_lost ?? losses;
        ...
        const levelClass = displayLevel ? `level-${displayLevel.toLowerCase()}` : '';
                    ...
                    <tr class="standings-player-row">
                        ...
                        <th scope="row">
                            <div class="player-name-cell">
                                <a href="/pages/player-profile.html?id=${p.id}&league_id=${state.leagueId}" class="player-name-link" onclick="event.stopPropagation()">${p.name}</a>
                                ${displayLevel ? `<span class="player-level-badge ${levelClass}" aria-label="${isFillIn ? 'Fill-in player' : `Level ${displayLevel}`}">${displayLevel}</span>` : ''}
                            </div>
                        </th>
                        <td class="stat-value wins" style="text-align: center; font-size: 14px;">${playerMatchWins}</td>
                        <td class="stat-value losses" style="text-align: center; font-size: 14px;">${playerMatchLosses}</td>
