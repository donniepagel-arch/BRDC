...
const TRIPLES_CACHE_KEY = `brdc:vnext:league:triples:${LEAGUE_ID}:v8`;
...
function ensurePlayoffRounds(rounds, seeds, playoffMatches) {
...
function renderPlayoffs() {
    ...
    const rounds = ensurePlayoffRounds(bracket.rounds, seeds, playoffMatches);
...
function renderPlayoffMatch(match, matchesByKey) {
    ...
    const href = match.id && state.matches.some(row => row.id === match.id)
        ? `/rookies/pages/match-hub-vnext.html?league_id=${LEAGUE_ID}&match_id=${encodeURIComponent(match.id)}`
        : '';
...
function renderSnapshot() {
    ...
    const nextHref = nextMatch
        ? `/rookies/pages/match-hub-vnext.html?league_id=${LEAGUE_ID}&match_id=${encodeURIComponent(nextMatch.id)}`
        : '/rookies/pages/messages-vnext.html';
...
function renderSchedule() {
    ...
    els.schedule.innerHTML = weekMatches.map(match => {
        ...
        return `
            <a class="tv-match-row" href="/rookies/pages/match-hub-vnext.html?league_id=${LEAGUE_ID}&match_id=${escapeHtml(match.id)}">
...
function playerStatRow(player) {
    ...
    return {
        ...
        highMark: Number(s.cricket_high_mark_round || 0),
...
function renderStatsPlayerRow(row, index, page) {
    ...
    return `<tr>
        ...
        <td>
            <a class="tv-stats-name" href="/rookies/pages/player-profile-vnext.html?player_id=${escapeHtml(row.id)}&league_id=${LEAGUE_ID}">
...
function renderStatsTeamRow(row, index, page) {
    ...
    const playerLines = row.players.map(player => `<tr class="tv-stats-player-subrow ${expanded ? '' : 'hidden'}">
        <td></td>
        <td><a class="tv-stats-name" href="/rookies/pages/player-profile-vnext.html?player_id=${escapeHtml(player.id)}&league_id=${LEAGUE_ID}"><span>${escapeHtml(player.level)}</span><strong>${escapeHtml(player.name)}</strong></a></td>
        ...
        return `
            <article class="tv-team-card">
                <header>
                    ...
                    <a class="tv-pill" href="/rookies/pages/league-team-vnext.html?league_id=${LEAGUE_ID}&team_id=${escapeHtml(team.id)}">Team page</a>
                </header>
    ...
    els.fillins.innerHTML = groups.map(level => {
        ...
        return `
            <section class="tv-fillin-level">
                ...
                ${rows.map(player => `
                    <a class="tv-fillin-row" href="/rookies/pages/player-profile-vnext.html?player_id=${escapeHtml(player.id)}&league_id=${LEAGUE_ID}">
...
function renderManage() {
    ...
    const matchRows = nextMatches.map(match => `
        <a class="tv-manage-row ${isPlayoffMatch(match) ? 'is-hot' : ''}" href="/rookies/pages/match-hub-vnext.html?league_id=${LEAGUE_ID}&match_id=${encodeURIComponent(match.id)}">
    ...
    els.manage.innerHTML = `
        ...
        <a class="tv-manage-panel tv-manage-action" href="/rookies/pages/league-import-vnext.html?league_id=${LEAGUE_ID}">
            <p class="tv-kicker">Results</p>
            <h3>Import Reports</h3>
            <p>Bring in DartConnect reports, review score corrections, and keep standings current.</p>
            <span>Open results tools</span>
        </a>

        <a class="tv-manage-panel tv-manage-action" href="/rookies/pages/contact-center-vnext.html?audience=league&league_id=${LEAGUE_ID}">
            <p class="tv-kicker">Contact</p>
            <h3>Contact Players</h3>
            <p>Send site messages, texts, or email to league players, captains, or custom groups.</p>
        ...
        <a class="tv-manage-panel tv-manage-action" href="/rookies/pages/create-league-vnext.html?league_id=${LEAGUE_ID}">
            <p class="tv-kicker">Settings</p>
            <h3>League Setup</h3>
            <p>Review format, schedule setup, playoff rules, roster rules, and registration details.</p>
            <span>Open setup</span>
        </a>

        <a class="tv-manage-panel tv-manage-action" href="/rookies/pages/director-home-vnext.html">
            <p class="tv-kicker">Admin</p>
            <h3>Admin Portal</h3>
            <p>Return to all Rookies leagues, events, series, and create-new actions.</p>
