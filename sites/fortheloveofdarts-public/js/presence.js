...
(function() {
    ...
    let lastPresenceAuthToken = null;
    ...
    async function sendHeartbeat() {
        ...
        try {
            const { callFunction, auth, waitForAuthReady } = await import('/js/firebase-config.js');
            const user = auth.currentUser || await waitForAuthReady(5000);
            if (user) {
                lastPresenceAuthToken = await user.getIdToken(false);
            }
    ...
    async function handleUnload() {
        ...
        // Keepalive preserves the authenticated request during navigation/unload.
        // Falling back to beacon without auth causes CORS/auth noise and is ignored.
        if (!lastPresenceAuthToken) return;

        fetch('https://us-central1-brdc-v2.cloudfunctions.net/setOffline', {
            method: 'POST',
            keepalive: true,
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${lastPresenceAuthToken}`
            },
            body: JSON.stringify({ player_id: playerId })
        }).catch(() => {
            // Page is unloading; there is nothing useful to surface here.
        });
    }
