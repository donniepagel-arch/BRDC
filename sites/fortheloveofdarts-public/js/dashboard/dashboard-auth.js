...
import { callFunction, db, auth, waitForAuthReady } from '/js/firebase-config.js';
...
async function initDashboardStable() {
    const savedSession = localStorage.getItem('brdc_session');
    ...
    const user = auth.currentUser || await waitForAuthReady(1500);
    if (!user) {
        localStorage.removeItem('brdc_session');
        showLoginPage();
        return;
