...
async function getLeaguePlayerStatsMap(leagueId) {
    ...
    try {
        const statsSnaps = [
            await getDocs(collection(db, 'leagues', leagueId, 'stats')),
            await getDocs(collection(db, 'leagues', leagueId, 'player_stats'))
        ];
        statsSnaps.forEach(statsSnap => statsSnap.forEach(docSnap => {
            ...
            candidateNames.forEach(name => {
                ...
                if (normalized) statsMap.set(normalized, merged);
            });
        }));
    } catch (error) {
        console.warn('Failed to load league player stats for modal hydration', leagueId, error?.message || error);
    }
...
function hydrateRosterWithStats(roster = [], statsMap = new Map()) {
    return roster.map(player => {
        ...
        const byName = statsMap.get(normalizePlayerName(player.name));
        const stats = byId || byName;
        if (!stats) return player;

        return {
            ...player,
            x01_three_dart_avg: player.x01_three_dart_avg ?? player.avg_3da ?? stats.x01_three_dart_avg ?? stats.avg_3da ?? null,
            cricket_mpr: player.cricket_mpr ?? player.mpr ?? stats.cricket_mpr ?? stats.mpr ?? null,
            avg_3da: player.avg_3da ?? stats.avg_3da ?? stats.x01_three_dart_avg ?? null,
            mpr: player.mpr ?? stats.mpr ?? stats.cricket_mpr ?? null
        };
    });
}

function formatRosterStat(value, decimals) {
    const num = Number(value);
    return Number.isFinite(num) ? num.toFixed(decimals) : '-';
}

function toFiniteRosterStat(value) {
    const num = Number(value);
                    ...
                    if ((team.league_id || team.id) === leagueId) {
                        ...
                        if (result.success) {
                            ...
                            if (matchData) {
                                ...
                                const statsMap = await getLeaguePlayerStatsMap(leagueId);
                                if (matchData.my_team?.roster) {
                                    matchData.my_team.roster = hydrateRosterWithStats(matchData.my_team.roster, statsMap);
                                }
                                if (matchData.opponent?.roster) {
                                    matchData.opponent.roster = hydrateRosterWithStats(matchData.opponent.roster, statsMap);
                                }
                                break;
                            }
                        }
...
function renderMatchPreviewModal(match, leagueId) {
    ...
    for (let i = 0; i < maxPlayers; i++) {
        ...
        const home3DA = homePlayer ? formatRosterStat(homePlayer.x01_three_dart_avg ?? homePlayer.avg_3da, 1) : '';
        const homePlayerMpr = homePlayer ? formatRosterStat(homePlayer.cricket_mpr ?? homePlayer.mpr, 2) : '';
        const away3DA = awayPlayer ? formatRosterStat(awayPlayer.x01_three_dart_avg ?? awayPlayer.avg_3da, 1) : '';
        const awayPlayerMpr = awayPlayer ? formatRosterStat(awayPlayer.cricket_mpr ?? awayPlayer.mpr, 2) : '';
...
function showMatchNightBanner(match) {
    ...
    const compareStats = (leftVal, rightVal) => {
        const leftNum = toFiniteRosterStat(leftVal);
        const rightNum = toFiniteRosterStat(rightVal);
        if (leftNum == null || rightNum == null) return { left: '', right: '' };
        if (Math.abs(leftNum - rightNum) < 0.1) return { left: '', right: '' };
        return leftNum > rightNum ? { left: 'better', right: 'worse' } : { left: 'worse', right: 'better' };
    };
    ...
    if (maxRows > 0) {
        ...
        for (let i = 0; i < maxRows; i++) {
            ...
            if (myPlayer) {
                const p3DA = toFiniteRosterStat(myPlayer.x01_three_dart_avg ?? myPlayer.avg_3da);
                const pMPR = toFiniteRosterStat(myPlayer.cricket_mpr ?? myPlayer.mpr);
                const o3DA = toFiniteRosterStat(oppPlayer?.x01_three_dart_avg ?? oppPlayer?.avg_3da);
                const oMPR = toFiniteRosterStat(oppPlayer?.cricket_mpr ?? oppPlayer?.mpr);

                const daCompare = compareStats(p3DA, o3DA);
                const mprCompare = compareStats(pMPR, oMPR);

                const p3DAStr = formatRosterStat(p3DA, 1);
                const pMPRStr = formatRosterStat(pMPR, 2);
        ...
        for (let i = 0; i < maxRows; i++) {
            ...
            if (oppPlayer) {
                ...
                const o3DAStr = formatRosterStat(o3DA, 1);
                const oMPRStr = formatRosterStat(oMPR, 2);
