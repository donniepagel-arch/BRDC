...
function renderHero() {
    ...
    els.messageTeamLink.href = `/rookies/pages/messages-vnext.html?${params}`;
    els.classicCaptainLink.href = `/rookies/pages/league-import-vnext.html?league_id=${encodeURIComponent(currentLeagueId)}`;
}
...
function renderMatchNight() {
    ...
    els.matchNightCard.innerHTML = `
        ...
        <div class="cv-action-row">
            <a href="/rookies/pages/match-hub-vnext.html?league_id=${encodeURIComponent(currentLeagueId)}&match_id=${encodeURIComponent(match.id)}">${isCompleted ? 'View match' : 'Open match hub'}</a>
            <button type="button" data-open-fillin>Find fill-in</button>
            <a href="/rookies/pages/messages-vnext.html?league_id=${encodeURIComponent(currentLeagueId)}&team_id=${encodeURIComponent(currentTeamId || '')}">Team chat</a>
            <a href="/rookies/pages/league-import-vnext.html?league_id=${encodeURIComponent(currentLeagueId)}" class="secondary" title="Open match report import tools">Import report</a>
        </div>
    `;
...
function renderSchedule() {
    ...
    els.scheduleList.innerHTML = matches.map(match => {
        ...
        const oppScore = match.is_home ? number(match.away_score) : number(match.home_score);
        return `
            <a class="cv-schedule-row" href="/rookies/pages/match-hub-vnext.html?league_id=${encodeURIComponent(currentLeagueId)}&match_id=${encodeURIComponent(match.id)}">
...
async function init() {
    ...
    if (!auth.currentUser) {
        els.captainStatus.textContent = 'Log in from the dashboard first.';
        return;
    }

    const sessionResult = await callFunction('getPlayerSession', {});
    if (!sessionResult?.success || !sessionResult.player?.id) throw new Error(sessionResult?.error || 'No player session');
