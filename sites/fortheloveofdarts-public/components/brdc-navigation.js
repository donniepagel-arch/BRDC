...
class BRDCNavigation {
    constructor(options = {}) {
        ...
        this.isVNextPreview = options.vnextPreview === true || window.location.pathname.includes('-vnext') || window.location.pathname.startsWith('/rookies');
        this._hiddenHeader = null; // track original header hidden by mobile header
    }

    route(page) {
        const routes = this.isVNextPreview ? {
            home: '/rookies/dashboard/',
            publicHome: '/rookies/',
            events: '/rookies/pages/events-vnext.html',
            chat: '/rookies/pages/messages-vnext.html',
            trader: '/rookies/pages/dart-trader-vnext.html',
            profile: '/rookies/pages/player-profile-vnext.html',
            admin: '/rookies/pages/director-home-vnext.html'
        } : {
            home: '/rookies/',
            events: '/rookies/pages/events-vnext.html',
            chat: '/rookies/pages/messages-vnext.html',
    ...
    renderMobileNav() {
        ...
        nav.innerHTML = `
            <a href="${this.route('home')}" class="mobile-nav-item ${this.currentPage === 'home' ? 'active' : ''}" data-page="home" ${this.currentPage === 'home' ? 'aria-current="page"' : ''}>
                <span class="mobile-nav-icon" aria-hidden="true">🏠</span>
                <span class="mobile-nav-label">Home</span>
            </a>
            <a href="${this.route('events')}" class="mobile-nav-item ${this.currentPage === 'events' ? 'active' : ''}" data-page="events" ${this.currentPage === 'events' ? 'aria-current="page"' : ''}>
                <span class="mobile-nav-icon" aria-hidden="true">📅</span>
                <span class="mobile-nav-label">Events</span>
            </a>
            <a href="${this.route('chat')}" class="mobile-nav-item ${this.currentPage === 'chat' ? 'active' : ''}" data-page="chat" ${this.currentPage === 'chat' ? 'aria-current="page"' : ''}>
            ...
            <a href="${this.route('profile')}" class="mobile-nav-item ${this.currentPage === 'profile' ? 'active' : ''}" data-page="profile" ${this.currentPage === 'profile' ? 'aria-current="page"' : ''}>
                <span class="mobile-nav-icon" aria-hidden="true">👤</span>
        ...
        header.innerHTML = `
            ...
            <div class="fb-header-center">
                <a href="${this.route('home')}">
                    <img src="https://static.wixstatic.com/media/e21a2d_89e369b22e004fdd8c3a7f3515ab7c22~mv2.png/v1/fill/w_360,h_90,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/Rookies%20Logo.png" alt="Rookies Sports Bar & Grill" class="fb-header-logo">
                </a>
            </div>
    ...
    renderDesktopNav() {
        ...
        nav.innerHTML = `
            <div class="desktop-nav-links desktop-nav-links-left">
                <a href="${this.route('home')}" class="desktop-nav-link ${this.currentPage === 'home' ? 'active' : ''}" data-page="home">
                    <span class="desktop-nav-link-icon">${icons.home}</span>
                    <span>Home</span>
                </a>
                <a href="${this.route('events')}" class="desktop-nav-link ${this.currentPage === 'events' ? 'active' : ''}" data-page="events">
                    <span class="desktop-nav-link-icon">${icons.events}</span>
                    <span>Events</span>
                </a>
                <a href="${this.route('chat')}" class="desktop-nav-link ${this.currentPage === 'chat' ? 'active' : ''}" data-page="chat">
                    <span class="desktop-nav-link-icon">${icons.chat}</span>
                    <span>Chat</span>
                </a>
            </div>
            <a href="${this.route('home')}" class="desktop-nav-logo">
                <img src="https://static.wixstatic.com/media/e21a2d_89e369b22e004fdd8c3a7f3515ab7c22~mv2.png/v1/fill/w_360,h_90,al_c,q_85,usm_0.66_1.00_0.01,enc_avif,quality_auto/Rookies%20Logo.png" alt="Rookies Sports Bar & Grill" height="32">
            </a>
            <div class="desktop-nav-links desktop-nav-links-right">
                <a href="${this.route('trader')}" class="desktop-nav-link ${this.currentPage === 'trader' ? 'active' : ''}" data-page="trader">
                ...
                <a href="${this.route('profile')}" class="desktop-nav-link ${this.currentPage === 'profile' ? 'active' : ''}" data-page="profile">
...
class NotificationsPanel {
    ...
    getSession() {
        try {
            ...
            return { ...session, player_id: playerId, id: playerId };
        } catch (error) {
            return null;
        }
    }
    ...
    getDemoNotifications() {
        return [
            {
                id: 'demo-playoff-ready',
                type: 'match',
                message: 'Playoff match tools are ready for Team K. Yasenchak vs Team J. Ragnoni.',
            ...
            },
            {
                id: 'demo-wing-series',
                type: 'event',
                message: 'Wing It Wednesdays is set up as a summer series with registration, voting, and bracket tools.',
            ...
            },
            {
                id: 'demo-no-write',
                type: 'system',
                message: 'Rookies demo mode is active. Admin actions preview the workflow without writing live changes.',
    ...
    async loadNotifications() {
        ...
        try {
            ...
            if (session.demo_mode === true && session.source_type === 'rookies_demo') {
    ...
    async markAsRead(id) {
        try {
            const session = this.getSession();
            if (session?.demo_mode === true) return;
