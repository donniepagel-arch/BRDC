/**
 * Import Week 2 - Russano v Ragnoni match from RTF file to Firestore
 * Run: node import-week2-russano-ragnoni.js
 */

const { parseMultiMatchRTF } = require('./parse-rtf.js');
...
// D. Russano = HOME, J. Ragnoni = AWAY
const TEAM_ROSTERS = {
    'D. Russano': ['Danny Russano', 'Chris Russano', 'Eric D', 'Eric Duale'],
    'J. Ragnoni': ['John Ragnoni', 'Marc Tate', 'Dave Brunner', 'David Brunner', 'Luke Kollias', 'Derek Fess', 'Josh Kelly']
};
...
function getTeamForPlayer(playerName, homeTeam, awayTeam) {
    ...
    console.warn(`  WARNING: Could not map player "${playerName}" to a team`);
    return null;
}
...
function convertToFirestoreFormat(allMatches, homeTeam, awayTeam) {
    const games = [];
    let totalDarts = 0;
    let totalLegs = 0;
    let homeScore = 0;
    ...
    // Process each match section (from different boards/tables)
    for (const matchSection of allMatches) {
        console.log(`\nProcessing match section ${matchSection.matchIndex} (${matchSection.games.length} games)`);

        for (const game of matchSection.games) {
            globalSetNumber++;
            const setMap = {
                set: globalSetNumber,
                legs: [],
            ...
            for (const leg of game.legs) {
                ...
                // For cricket, use higher score or parser winner
                if (!winner && legType.includes('cricket')) {
                    ...
                    // Fallback: compare scores from final throw
                    if (!winner) {
                        const lastThrow = throws[throws.length - 1];
                        if (lastThrow) {
                            // Get home and away final scores
                            let homeScore = 0, awayScore = 0;
                            for (const t of throws) {
                                const side = getTeamForPlayer(t.player, homeTeam, awayTeam);
                                if (side === 'home' && t.score > homeScore) homeScore = t.score;
                                if (side === 'away' && t.score > awayScore) awayScore = t.score;
                            }
                            if (homeScore > awayScore) winner = 'home';
                            else if (awayScore > homeScore) winner = 'away';
                        }
                    }
                    // Last resort: check for closing throw
                    if (!winner) {
                ...
                if (winner === 'home') setMap.homeLegsWon++;
                else if (winner === 'away') setMap.awayLegsWon++;
                ...
                setMap.legs.push(legData);
            }

            // Determine set winner
            const gameWinner = setMap.homeLegsWon > setMap.awayLegsWon ? 'home' :
                              setMap.awayLegsWon > setMap.homeLegsWon ? 'away' : 'tie';
            ...
            games.push({
                game_number: globalSetNumber,
                set: globalSetNumber,
                type: setMap.legs[0]?.format || 'mixed',
                format: setMap.legs[0]?.format || 'mixed',
                home_players: Array.from(homePlayersSet),
                away_players: Array.from(awayPlayersSet),
                result: {
                    home_legs: setMap.homeLegsWon,
                    away_legs: setMap.awayLegsWon
                },
                winner: gameWinner,
                status: 'completed',
                legs: setMap.legs
            ...
            console.log(`  Set ${globalSetNumber}: ${Array.from(homePlayersSet).join('/')} vs ${Array.from(awayPlayersSet).join('/')} => ${setMap.homeLegsWon}-${setMap.awayLegsWon} (${gameWinner})`);
        }
    }
...
async function importMatch() {
    ...
    const rtfPath = path.join(__dirname, 'trips league/week 2/russano v ragnoni.rtf');
    console.log(`Reading: ${rtfPath}`);

    try {
        // Use parseMultiMatchRTF to handle multiple match sections
        let allMatches = parseMultiMatchRTF(rtfPath);
        console.log(`Parsed ${allMatches.length} match sections`);

        const matchData = convertToFirestoreFormat(allMatches, homeTeam, awayTeam);
        console.log(`\nConverted to ${matchData.games.length} sets, ${matchData.total_legs} legs`);
        console.log(`Final Score: ${homeTeam} ${matchData.final_score.home} - ${matchData.final_score.away} ${awayTeam}`);
